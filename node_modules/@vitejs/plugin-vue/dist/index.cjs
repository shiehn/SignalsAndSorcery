...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  if (ssr) {
    const normalizedFilename = pluginutils.normalizePath(path__default.relative(options.root, filename));
  ...
  if ((descriptor.script?.lang === "ts" || descriptor.scriptSetup?.lang === "ts") && !descriptor.script?.src) {
    const { code: code2, map: map2 } = await vite.transformWithEsbuild(resolvedCode, filename, { loader: "ts", sourcemap: options.sourceMap }, resolvedMap);
    resolvedCode = code2;
...
async function transformStyle(code, descriptor, index, options, pluginContext, filename) {
  ...
  const map = result.map ? vite.formatPostcssSourceMap(result.map, filename) : { mappings: "" };
  return {
    code: result.code,
    map
  };
...
function vuePlugin(rawOptions = {}) {
  ...
  const customElementFilter = typeof customElement === "boolean" ? () => customElement : pluginutils.createFilter(customElement);
  const refTransformFilter = reactivityTransform === false ? () => false : reactivityTransform === true ? pluginutils.createFilter(/\.(j|t)sx?$/, /node_modules/) : pluginutils.createFilter(reactivityTransform);
  let options = {
  ...
  const isSSR = (opt) => opt === void 0 ? false : typeof opt === "boolean" ? opt : opt?.ssr === true;
  return {
    name: "vite:vue",
    handleHotUpdate(ctx) {
      if (!filter(ctx.file)) {
