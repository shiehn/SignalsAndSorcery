import fs from 'fs';
import { normalizePath as normalizePath$1, createFilter } from '@rollup/pluginutils';
import { createRequire } from 'module';
import path from 'path';
import { createHash } from 'crypto';
import { transformWithEsbuild, formatPostcssSourceMap } from 'vite';
...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  if ((descriptor.script?.lang === "ts" || descriptor.scriptSetup?.lang === "ts") && !descriptor.script?.src) {
    const { code: code2, map: map2 } = await transformWithEsbuild(resolvedCode, filename, { loader: "ts", sourcemap: options.sourceMap }, resolvedMap);
    resolvedCode = code2;
...
async function transformStyle(code, descriptor, index, options, pluginContext, filename) {
  ...
  const map = result.map ? formatPostcssSourceMap(result.map, filename) : { mappings: "" };
  return {
    code: result.code,
    map
  };
